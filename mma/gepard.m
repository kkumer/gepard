...
(* Wilson coefficient "bigC" multiplied by evolution operator with scale ratio R *)
(* ----------------------------------------------------------------------------- *)

cDVCS[{(j_)?NumericQ, rf_, rr_, (R_)?NumericQ}, 
     {sector_, flavor_ /; flavor == SP}, {nf_, order_, 
      scheme_ /; (scheme == bar2CS || scheme == MS)}, {c___, eval_ /; eval == GEP}] := 
      Sum[cDVCS[{j, rf, rr}, {sector, flavor}, {nf, scheme}, {eval}][[
         order - ord + 1]] . RAope[{j, R}, {sector, flavor}, {nf, scheme}, 
          {eval}][[ord + 1]], {ord, 0, order}]


(* Particular order in expansion of j-th partial wave. *) 
(* --------------------------------------------------- *)

(* Calls input GPD Fcm[] which has to be already defined somewhere. *)

Fpw[{(j_)?NumericQ, (xi_)?NumericQ, Delta2_, (Q2_)?NumericQ, {Q02_}}, 
     {{F_, flavor_ /; flavor == SP}, argAns__}, {nf_, {appr_, order_}, 
      {scheme_, rf_:1, rr_:1}}, {c___, eval_ /; eval == GEP || 
        eval == GEPI || eval == GEPIfix}] := 
    cDVCS[{j, rf, rr, AS2P[Q2/rf, nf, NP2O[appr]]/AS2P[Q02, nf, NP2O[appr]]}, 
      {GPD2Sector[F], flavor}, {nf, order, scheme}, {c, eval}] . 
     Fcm[{j, xi, Delta2, Q02}, {{F, flavor}, argAns}, {appr, scheme, rf, rr}]
 

(* Complete j-th partial wave, up to some order appr=LO, NLO or NNLO *)
(* ----------------------------------------------------------------- *)

(* For GEP we need initialization of common blocks so we define separate Fpw *)
Fpw[{(j_)?NumericQ, (xi_)?NumericQ, Delta2_, (Q2_)?NumericQ, 
      {(Q02_)?NumericQ}}, {argAns__}, {nf_, appr_, {scheme_, rf_:1, rr_:1}}, 
     {c___, eval_ /; eval == GEP}] := 
    Block[{}, common[j + 1, nf]; Sum[AS2P[Q2/rr, nf, NP2O[appr]]^order*
        Fpw[{j, xi, Delta2, Q2, {Q02}}, {argAns}, {nf, {appr, order}, 
          {scheme, rf, rr}}, {c, eval}], {order, 0, NP2O[appr]}]]


(* All the rest *)
Fpw[{argVar__, Q2_, {Q02_}}, {argAns__}, {nf_, LO, 
      {scheme_, rf_:1, rr_:1}}, {c_, eval__ /; eval==GEPI || eval==GEPIfix}] = 
Fpw[{argVar, Q2, {Q02}}, {argAns}, {nf, {LO, 0}, {scheme, rf, rr}}, {c, eval}]

Fpw[{argVar__, Q2_, {Q02_}}, {argAns__}, {nf_, NLO, 
      {scheme_, rf_:1, rr_:1}}, {c_, eval__ /; eval==GEPI || eval==GEPIfix}] = 
Sum[AS2P[Q2/rr, nf, 1]^ord * Fpw[{argVar, Q2, {Q02}}, {argAns}, 
       {nf, {NLO, ord}, {scheme, rf, rr}}, {c, eval}], {ord,0,1}]

Fpw[{argVar__, Q2_, {Q02_}}, {argAns__}, {nf_, NNLO, 
      {scheme_, rf_:1, rr_:1}}, {c_, eval__ /; eval==GEPI || eval==GEPIfix}] = 
Sum[AS2P[Q2/rr, nf, 2]^ord * Fpw[{argVar, Q2, {Q02}}, {argAns}, 
       {nf, {NNLO, ord}, {scheme, rf, rr}}, {c, eval}], {ord,0,2}]
(* ToString[eval] above is because != will not evaluate to False for symbols *)
