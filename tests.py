...
def test_Xunp():
    assert_almost_equal(b.Xunp(pt0, DMGLO1, {'phi':1.}), 1.8934179005138172)
    ar = b.Xunp(pt0, DMGLO1, {'phi':np.array([0.3, 1.1])})
...
def test_Xunp3():
    ...
    ar = b.Xunp(pt0, DMGLO1, {'phi':1, 'Q2':np.array([2.3, 2.5])})
    assert isinstance(ar, np.ndarray)
...
def test_Xunp4():
    ...
    ar = b.Xunp(pt0, DMGLO1, {'phi':1, 'xB':np.array([0.07, 0.11])})
    assert isinstance(ar, np.ndarray)
