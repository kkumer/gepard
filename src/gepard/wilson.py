...
def calc_wc_dvmp(m: theory.Theory, j: np.ndarray, g: int):
    """Calculate Wilson coeffs for DVMP.

    Args:
        m: instance of the Theory class
        j: GPD MB contour point(s) (overrides m.jpoints)
...
def calc_wce(m: theory.Theory, Q2: float, process_class: str):
    ...
    for pw_shift in [0, 2, 4]:
        ...
        evola = evolution.evolop(m, j, R, process_class)[..., :3, :3]   # 3x3
...
def calc_wce_dvmp(m: theory.Theory, Q2: float):
    ...
    for pw_shift in [0, 2, 4]:
        ...
        evola_da = evolution.evolop(m, np.array(m.gpoints, dtype=complex),
                                    RDA, process_class, DA=True)[..., 0, 0] # 0->1 !!
        evola = evolution.evolop(m, j, R, process_class)[..., :3, :3]   # 3x3
        # p_mat: matrix that combines (LO, NLO) evolution operator and Wilson coeffs
        # while canceling NNLO term NLO*NLO:
...
def calc_j2x(m: theory.Theory, x: float, eta: float, Q2: float):
    ...
    for pw_shift in [0, 2, 4]:
        ...
        if eta < 1e-8:
            ...
            evola = evolution.evolop(m, j, R, 'DIS')[..., :3, :3]   # 3x3
        elif abs(eta-x) < 1e-8:
            ...
            evola = evolution.evolop(m, j, R, 'DVCS')[..., :3, :3]   # 3x3
        else:
