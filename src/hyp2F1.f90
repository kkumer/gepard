...
CONTAINS
  !
  FUNCTION INF_NORM(Z)
    COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z
    REAL(KIND(1.0D0))  :: INF_NORM
    INF_NORM=MAX(ABS(REAL(Z,KIND(1.0D0))),ABS(AIMAG(Z)))
    RETURN
  END FUNCTION INF_NORM
  !
  FUNCTION TANZ(Z)
    COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z
    COMPLEX(KIND(1.0D0)) :: TANZ
    TANZ=SIN(Z)/COS(Z)
    RETURN
  END FUNCTION TANZ
  !
  FUNCTION LOG1P(Z)
    COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z
    REAL(KIND(1.0D0)) :: X,XP1,LOG1P_X
    REAL(KIND(1.0D0)) :: Y,YX,YX2,YX2P1,LOG1P_YX2
    REAL(KIND(1.0D0)) :: RE_LOG1P,IM_LOG1P
    COMPLEX(KIND(1.0D0)) :: LOG1P
    IF(INF_NORM(Z).LT.ONE) THEN
       X = REAL(Z,KIND(1.0D0)); XP1 = X+ONE
       IF(XP1.EQ.ONE) THEN
          LOG1P_X = X
       ELSE
       ...
       LOG1P = CMPLX(RE_LOG1P,IM_LOG1P,KIND(1.0D0))
       RETURN
    ELSE
       LOG1P=LOG(ONE+Z)
       RETURN
  ...
  FUNCTION EXPM1(Z)
    COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z
    REAL(KIND(1.0D0)) :: X,EXPM1_X,EXP_X,Y,SIN_HALF_Y
    REAL(KIND(1.0D0)) :: RE_EXPM1,IM_EXPM1
    COMPLEX(KIND(1.0D0)) :: EXPM1
    IF(INF_NORM(Z).LT.ONE) THEN
       X = REAL(Z,KIND(1.0D0)); EXP_X = EXP(X)
...
!
!----------------------------------------------------------------------
RECURSIVE FUNCTION MYLOG_GAMMA(Z) RESULT(RES)
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
  ...
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z
  INTEGER(KIND(1)) :: N,I
  REAL(KIND(1.0D0))     :: X,Y,LOG_SQRT_2PI,G,LOG_PI,M_LN2,C(0:14)
  COMPLEX(KIND(1.0D0))  :: GAMMA_SUM,Z_M_0P5,Z_P_G_M0P5,ZM1
  COMPLEX(KIND(1.0D0))  :: LOG_CONST,I_PI,EPS,LOG_SIN_PI_Z,RES
  !
  M_LN2=0.69314718055994530942D0; X=REAL(Z,KIND(1.0D0)); Y=AIMAG(Z)
  IF((Z.EQ.NINT(X)).AND.(X.LE.ZERO)) &
       STOP 'Z IS NEGATIVE INTEGER IN MYLOG_GAMMA'
  IF(X.GE.HALF) THEN
  ...
  ELSE IF(Y.GE.ZERO) THEN
     ...
     LOG_CONST=CMPLX(-M_LN2,M_PI_2,KIND(1.0D0)); I_PI=CMPLX(ZERO,M_PI,KIND(1.0D0))
     EPS=Z-N
     ...
     ENDIF
     RES=LOG_PI-LOG_SIN_PI_Z-MYLOG_GAMMA(ONE-Z);
     RETURN
  ELSE
     RES=CONJG(MYLOG_GAMMA(CONJG(Z)))
     RETURN
  ENDIF
END FUNCTION MYLOG_GAMMA
!
!----------------------------------------------------------------------
...
RECURSIVE FUNCTION GAMMA_INV(Z) RESULT(RES)
  ...
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z
  INTEGER(KIND(1)) :: N,I
  REAL(KIND(1.0D0))     :: X,LOG_SQRT_2PI,G,C(0:14)
  COMPLEX(KIND(1.0D0))  :: RES,GAMMA_SUM,Z_M_0P5,Z_P_G_M0P5,ZM1,EPS
  !
  X=REAL(Z,KIND(1.0D0))
  IF(X.GE.HALF) THEN
     LOG_SQRT_2PI=0.91893853320467274177D0; G=4.7421875D0
...
RECURSIVE FUNCTION GAMMA_RATIO_DIFF_SMALL_EPS(Z,EPS) RESULT(RES)
  ...
  USE HYP_2F1_MODULE
  IMPLICIT NONE
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z,EPS
  INTEGER(KIND(1)) :: N,M,I
  REAL(KIND(1.0D0))     :: G,X,EPS_PX,C(0:14)
  COMPLEX(KIND(1.0D0))  :: RES,SUM_NUM,SUM_DEN
  COMPLEX(KIND(1.0D0))  :: EPS_PZ,Z_M_0P5,Z_PG_M0P5,EPS_PZ_PG_M0P5,ZM1
  COMPLEX(KIND(1.0D0))  :: CI_ZM1_PI_INV,PI_EPS,TT,T1_EPS_Z,SIN_PI_2_EPS
  COMPLEX(KIND(1.0D0))  :: ZM1_P_EPS,T2_EPS_Z,T_EPS_Z
  !
  G=4.74218750D0
  ...
  X=REAL(Z,KIND(1.0D0)); EPS_PX=REAL(EPS_PZ,KIND(1.0D0)); N=NINT(X); M=NINT(EPS_PX)
...
FUNCTION GAMMA_INV_DIFF_EPS(Z,EPS)
  ...
  IMPLICIT NONE
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z,EPS
  INTEGER(KIND(1)) :: M,N,K
  REAL(KIND(1.0D0))     :: X,EPS_PX,FACT
  REAL(KIND(1.0D0))     :: Z_NEG_INT_DISTANCE
  REAL(KIND(1.0D0))     :: EPS_PZ_NEG_INT_DISTANCE
  COMPLEX(KIND(1.0D0))  :: GAMMA_INV_DIFF_EPS,EPS_PZ,GAMMA_INV
  COMPLEX(KIND(1.0D0))  :: GAMMA_RATIO_DIFF_SMALL_EPS
  LOGICAL      :: IS_Z_NEG_INT,IS_EPS_PZ_NEG_INT

  EPS_PZ=Z+EPS; X=REAL(Z,KIND(1.0D0)); EPS_PX=REAL(EPS_PZ,KIND(1.0D0))
...
FUNCTION A_SUM_INIT(M,EPS,GAMMA_INV_ONE_MEPS)
  ...
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: EPS,GAMMA_INV_ONE_MEPS
  INTEGER(KIND(1)) :: N,I
  REAL(KIND(1.0D0))     :: FACT
  COMPLEX(KIND(1.0D0))  :: A_SUM_INIT,ONE_MEPS
  COMPLEX(KIND(1.0D0))  :: GAMMA_INV_ONE_MEPS_MM
  !
  ONE_MEPS = ONE - EPS
...
FUNCTION LOG_A_SUM_INIT(M,EPS)
  ...
  INTEGER(KIND(1)) :: N
  REAL(KIND(1.0D0))     :: LOG_FACT
  COMPLEX(KIND(1.0D0))  :: ONE_MEPS_MM,LOG_A_SUM_INIT,MYLOG_GAMMA
  !
  ONE_MEPS_MM=ONE-EPS-M
  IF(ONE_MEPS_MM.NE.1-M) THEN
     LOG_A_SUM_INIT=(-MYLOG_GAMMA(ONE_MEPS_MM) - LOG(EPS))
     RETURN
  ELSE
     LOG_FACT=ZERO
     DO N=2,M-1
     ...
     ELSE
        LOG_A_SUM_INIT=CMPLX(LOG_FACT,M_PI,KIND(1.0D0))
     ENDIF
     RETURN
  ENDIF
...
FUNCTION B_SUM_INIT_PS_ONE(A,B,GAMMA_C,GAMMA_INV_ONE_MEPS, &
  ...
  IMPLICIT NONE
  INTEGER(KIND(1)),INTENT(IN) :: M
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: A,B,GAMMA_C,GAMMA_INV_ONE_MEPS, &
       GAMMA_INV_EPS_PA_PM,GAMMA_INV_EPS_PB_PM,MZP1,EPS
  INTEGER(KIND(1)) :: M_M1,N,I,PHASE
  REAL(KIND(1.0D0))     :: INF_NORM_EPS,GAMMA_INV_MP1
  COMPLEX(KIND(1.0D0))  :: A_PM,B_SUM_INIT_PS_ONE,PI_EPS,GAMMA_INV_ONE_MEPS_MM
  COMPLEX(KIND(1.0D0))  :: B_PM,TMP1,TMP2
...
FUNCTION B_SUM_INIT_PS_INFINITY(A,C,GAMMA_C,GAMMA_INV_CMA, &
  ...
  IMPLICIT NONE
  INTEGER(KIND(1)),INTENT(IN) :: M
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: A,C,GAMMA_C,GAMMA_INV_CMA,Z,EPS
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: GAMMA_INV_ONE_MEPS,GAMMA_INV_EPS_PA_PM
  INTEGER(KIND(1)) :: M_M1,I,N,N0,PHASE
  LOGICAL      :: IS_N0_HERE,IS_EPS_NON_ZERO
  REAL(KIND(1.0D0))     :: INF_NORM_EPS,NP1,GAMMA_INV_MP1
  COMPLEX(KIND(1.0D0))  :: B_SUM_INIT_PS_INFINITY,GAMMA_INV,TMP1
  COMPLEX(KIND(1.0D0))  :: CMA,A_MC_P1,A_MC_P1_PM,CMA_MEPS,EPS_PA_MC_P1,A_PM
  COMPLEX(KIND(1.0D0))  :: GAMMA_INV_EPS_PM_P1,GAMMA_INV_CMA_MEPS,PI_EPS
  COMPLEX(KIND(1.0D0))  :: PROD1,PROD2,A_PN,A_MC_P1_PN,ONE_MEPS
  COMPLEX(KIND(1.0D0))  :: PROD_A,PROD_A_MC_P1,PROD_EPS_PA_MC_P1_N0,PI_EPS_PM
  COMPLEX(KIND(1.0D0))  :: PROD_EPS_PA_MC_P1,SUM_N0,Z_TERM,SUM_TERM
  COMPLEX(KIND(1.0D0))  :: PROD_DIFF_EPS,GAMMA_INV_A_PM,GAMMA_PROD1
  COMPLEX(KIND(1.0D0))  :: PROD_2A,PROD_2B,PROD_2C,GAMMA_INV_DIFF_EPS
  COMPLEX(KIND(1.0D0))  :: EPS_PA_MC_P1_PN,GAMMA_INV_ONE_MEPS_MM
  !
...
SUBROUTINE CV_POLY_DER_TAB_CALC(A,B,C,Z,CV_POLY_DER_TAB)
  ...
  REAL(KIND(1.0D0)),INTENT(OUT) :: CV_POLY_DER_TAB(0:3)
  REAL(KIND(1.0D0))     :: MOD_A2,MOD_B2,MOD_C2,MOD_Z2
  REAL(KIND(1.0D0))     :: RE_A,RE_B,RE_C,IM_A,IM_B,IM_C,RE_Z,IM_Z
  !
  RE_A=REAL(A,KIND(1.0D0)); IM_A=AIMAG(A); MOD_A2=RE_A*RE_A+IM_A*IM_A
  RE_B=REAL(B,KIND(1.0D0)); IM_B=AIMAG(B); MOD_B2=RE_B*RE_B+IM_B*IM_B
  RE_C=REAL(C,KIND(1.0D0)); IM_C=AIMAG(C); MOD_C2=RE_C*RE_C+IM_C*IM_C
  RE_Z=REAL(Z,KIND(1.0D0)); IM_Z=AIMAG(Z); MOD_Z2=RE_Z*RE_Z+IM_Z*IM_Z
...
FUNCTION CV_POLY_DER_CALC(CV_POLY_DER_TAB,X)
  ...
  REAL(KIND(1.0D0)),INTENT(IN) :: CV_POLY_DER_TAB(0:3)
  REAL(KIND(1.0D0)) :: CV_POLY_DER_CALC
  !
...
FUNCTION MIN_N_CALC(CV_POLY_DER_TAB)
  ...
  IMPLICIT NONE
  REAL(KIND(1.0D0)),INTENT(IN) :: CV_POLY_DER_TAB(0:3)
  INTEGER(KIND(1)) :: MIN_N_CALC
  REAL(KIND(1.0D0))     :: C1,C2,THREE_C3,DELTA,LARGEST_ROOT
  !
...
FUNCTION HYP_PS_ZERO(A,B,C,Z)
  ...
  IMPLICIT NONE
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: A,B,C,Z
  INTEGER(KIND(1)) :: N,NA,NB,MIN_N,MIN_N_CALC
  COMPLEX(KIND(1.0D0))  :: HYP_PS_ZERO,TERM
  LOGICAL :: POSSIBLE_FALSE_CV
  REAL(KIND(1.0D0)) :: CV_POLY_DER_TAB(0:3)
  REAL(KIND(1.0D0)) :: CV_POLY_DER_CALC
  !
...
FUNCTION HYP_PS_ONE(A,B,C,MZP1)
  ...
  INTEGER(KIND(1)) :: N,M,PHASE,M_M2,MIN_N,MIN_N_CALC,M_P1
  ...
  COMPLEX(KIND(1.0D0))  :: A_SUM_INIT,LOG_A_SUM_INIT,A_SUM,A_TERM,ONE_MEPS
  COMPLEX(KIND(1.0D0))  :: B_EXTRA_TERM,B_TERM,B_SUM,GAMMA_C,MYLOG_GAMMA,RATIO
  COMPLEX(KIND(1.0D0))  :: A_PM_PN,B_PM_PN,EPS_PM_P1_PN,N_P1_MEPS
  COMPLEX(KIND(1.0D0))  :: PROD1,PROD2,PROD3
  COMPLEX(KIND(1.0D0))  :: EPS_PA_PM_PN,EPS_PB_PM_PN,EPS_PM_PN,PROD_B,POW_MZP1_M
  COMPLEX(KIND(1.0D0))  :: GAMMA_INV_EPS_PA_PM,GAMMA_INV_EPS_PB_PM
  COMPLEX(KIND(1.0D0))  :: GAMMA_INV_ONE_MEPS
  LOGICAL :: POSSIBLE_FALSE_CV
  REAL(KIND(1.0D0)) :: CV_POLY1_DER_TAB(0:3),CV_POLY2_DER_TAB(0:3)
  !
  M=NINT(REAL(C-A-B,KIND(1.0D0))); M_M2=M-2; M_P1=M+1
  IF(MOD(M,2).EQ.0) THEN
     PHASE=1
  ELSE
     PHASE=-1
  ...
  ELSE
     A_TERM=EXP(MYLOG_GAMMA(C)-MYLOG_GAMMA(EPS_PA_PM)&
          -MYLOG_GAMMA(EPS_PB_PM)+LOG_A_SUM_INIT(M,EPS))
...
FUNCTION HYP_PS_INFINITY(A,B,C,Z)
  ...
  INTEGER(KIND(1)) :: N,M,PHASE,M_M2,MIN_N,MIN_N_CALC,M_P1
  REAL(KIND(1.0D0))     :: B_PREC,N_P1,N_PM_P1,CV_POLY_DER_CALC
  COMPLEX(KIND(1.0D0))  :: B_SUM_INIT_PS_INFINITY,MYLOG_GAMMA,POW_Z_INV_M
  COMPLEX(KIND(1.0D0))  :: HYP_PS_INFINITY,Z_INV,GAMMA_INV,RATIO
  COMPLEX(KIND(1.0D0))  :: EPS,A_MC_P1,ONE_MEPS,ONE_MEPS_MM,A_PM,A_MC_P1_PM
  COMPLEX(KIND(1.0D0))  :: CMA,EPS_PA,EPS_PM_P1,EPS_PA_MC_P1_PM,PI_EPS
  COMPLEX(KIND(1.0D0))  :: EPS_PA_PM,EPS_PM,GAMMA_C,GAMMA_INV_CMA,POW_MZ_MA
  COMPLEX(KIND(1.0D0))  :: A_SUM_INIT,LOG_A_SUM_INIT,A_SUM,A_TERM
  COMPLEX(KIND(1.0D0))  :: GAMMA_INV_EPS_PA_PM,GAMMA_INV_ONE_MEPS
  COMPLEX(KIND(1.0D0))  :: PROD_B,B_EXTRA_TERM,B_TERM,B_SUM,PROD1
  COMPLEX(KIND(1.0D0))  :: A_PM_PN,A_MC_P1_PM_PN,EPS_PM_P1_PN,N_P1_MEPS
  COMPLEX(KIND(1.0D0))  ::   PROD2,PROD3,GAMMA_PROD
  COMPLEX(KIND(1.0D0))  :: EPS_PA_PM_PN,EPS_PA_MC_P1_PM_PN,EPS_PM_PN
  LOGICAL :: POSSIBLE_FALSE_CV
  REAL(KIND(1.0D0)) :: CV_POLY1_DER_TAB(0:3),CV_POLY2_DER_TAB(0:3)
  !
  M=NINT(REAL(B-A,KIND(1.0D0))); M_M2=M-2;M_P1=M+1
  IF(MOD(M,2).EQ.0) THEN
     PHASE=1
  ELSE
     PHASE=-1
  ...
  ELSE IF(INF_NORM(ONE_MEPS_MM &
     ...
     A_TERM=GAMMA_PROD*A_SUM_INIT(M,EPS,GAMMA_INV_ONE_MEPS)
  ELSE
     A_TERM=EXP(MYLOG_GAMMA(C)-MYLOG_GAMMA(CMA)-MYLOG_GAMMA(B) &
...
FUNCTION HYP_PS_COMPLEX_PLANE_REST(A,B,C,Z)
  ...
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: A,B,C,Z
  INTEGER(KIND(1)) :: N
  REAL(KIND(1.0D0))     :: ABS_Z,PREC
  COMPLEX(KIND(1.0D0))  :: HYP_PS_COMPLEX_PLANE_REST
  COMPLEX(KIND(1.0D0))  :: Z0,ZC,ZC_Z0_RATIO,Z0_TERM1,Z0_TERM2
  COMPLEX(KIND(1.0D0))  :: HYP_PS_Z0,DHYP_PS_Z0,AN,ANP1,ANP2
  COMPLEX(KIND(1.0D0))  :: HYP_PS_ZERO,HYP_PS_INFINITY
  !
  ABS_Z=ABS(Z)
...
RECURSIVE FUNCTION HYP_2F1(A,B,C,Z) RESULT(RES)
  ...
  IMPLICIT NONE
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: A,B,C,Z
  INTEGER(KIND(1)) :: NA,NB,NC,I
  REAL(KIND(1.0D0))     :: RE_A,RE_B,RE_C,ABS_Z,ABS_ZM1,ABS_Z_OVER_ZM1
  REAL(KIND(1.0D0))     :: ABS_ZM1_OVER_Z,ABS_ZM1_INV,R_TABLE(1:5),R,ABS_Z_INV
  COMPLEX(KIND(1.0D0))  :: RES,HYP_PS_INFINITY,HYP_PS_ZERO,Z_SHIFT
  COMPLEX(KIND(1.0D0))  :: HYP_PS_COMPLEX_PLANE_REST,HYP_PS_ONE,Z_OVER_ZM1,ZM1
  ...
  !
  RE_A=REAL(A,KIND(1.0D0)); RE_B=REAL(B,KIND(1.0D0)); RE_C=REAL(C,KIND(1.0D0));
  ...
  ELSE IF(IS_B_NEG_INT) THEN
     ...
     ENDIF
  ENDIF
  IF((REAL(Z,KIND(1.0D0)).GE.ONE).AND.(AIMAG(Z).EQ.ZERO)) THEN
     Z_SHIFT=CMPLX(REAL(Z,KIND(1.0D0)),-1.0D-307,KIND(1.0D0))
...
FUNCTION TEST_2F1(A,B,C,Z,F)
  ...
  IMPLICIT NONE
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: A,B,C,Z
  REAL(KIND(1.0D0))    :: TEST_2F1
  COMPLEX(KIND(1.0D0)) :: F,DF,D2F,HYP_2F1  
  !
  IF(Z.EQ.ZERO) THEN
...
FUNCTION MYTEST(Z)
  ...
  IMPLICIT NONE
  COMPLEX(KIND(1.0D0)),INTENT(IN) :: Z
  COMPLEX(KIND(1.0D0))    :: MYTEST, a
  COMPLEX(KIND(1.0D0)) :: HYP_2F1
  !
     a = (1.0d0, 3.0d0)
     MYTEST = HYP_2F1((1.0d0, 3.0d0), 3.0d0, 6.0d0, Z)
     RETURN
END FUNCTION MYTEST
!============== END HYP_2F1 FILE ======================================
