...
\label{eq:parsigma}
\end{equation}
where singlet Compton form factor $^{\rm S}\mathcal{H}$ is given by
\begin{equation}
...
\item
\texttt{AS0} --- \emph{Double precision}. Value of $\alpha_{\rm s}(2.5 \textrm{GeV}^2)/(2 \pi)$. For various
...
program execution, and values specified inside can be overriden later. For
...
printed out (e.g. by Minuit command \texttt{cali 3}), one gets two new files.
One is \texttt{FIT.OUT}, containing the numbers from each data file, together with
...
\hrule
\end{center}
\caption{Example of a part of resulting fit file \texttt{FIT.OUT}}
\label{fig:fitout}
\end{figure}

If one has PGPLOT library compiled in, than this fits are also graphed,
as specified at the beginning of the file \texttt{FIT.INI}. Example
graph is shown on Fig.~\ref{fig:fitps}
...
Calling graph of subroutines is displayed on Fig.~\ref{fig:callgraph}. Each subroutine,
