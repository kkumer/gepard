...
if len(sys.argv) != 2:
    sys.stdout.write('Usage: EIC.py <1 or 2>')
    sys.exit(1)
...
# Hybrid models - only one can be loaded
...
if sys.argv[1] == '1':
    ...
    ysKM10b = [thBM.TSA(ptcol(thBM, pol=0, Ep=150, xB=0.1, phi=f)) for f in phis]
    ...
    ysKM10bR = [thBM.TSA(ptcol(thBM, pol=0, Ep=350, xB=0.01, phi=f)) for f in phis]
