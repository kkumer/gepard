...
def test_lambda(th):
    ...
    # leading PW
    gam0 = g.adim.singlet_LO(th.jpoints_pws + 1, th.nf)
    lam = g.evolution.lambdaf(gam0)
    assert lam[0, 0, :] == approx(
           np.array([-22.79064075+0.01967868j, 3.56546819+0.00069647j]))
    # nl PW
    assert lam[1, 0, :] == approx(
...
def test_rnlof(th_nlo):
    ...
    # leading PW
    lam, pr, r1proj = g.evolution.rnlof(th_nlo)
    assert pr[0, 0, 0, :, :] == approx(
...
def test_evolop_LO(th_lo):
    ...
    assert g.evolution.evolop(th_lo, th_lo.R,
                              'DVCS')[0, 0, 0, :2, :2] == approx(
...
def test_evolop_NLO(th_nlo):
    ...
    evola = g.evolution.evolop(th_nlo, th_nlo.R, 'DVCS')
    # LO part (but with NLO alpha_strong)
    assert evola[0, 0, 0, :2, :2] == approx(
