...
def test_lambda(th):
    ...
    # leading PW
    gam0 = g.adim.block(th.jpoints+1, th.nf)[:, 0, :, :]
    assert g.evolution.lambdaf(gam0)[:2, 0] == approx(
           np.array([-22.79064075+0.01967868j, 3.56546819+0.00069647j]))
    # nl PW
    gam0 = g.adim.block(th.jpoints+3, th.nf)[:, 0, :, :]
    assert g.evolution.lambdaf(gam0)[:2, 0] == approx(
...
def test_rnlof(th_nlo):
    ...
    assert r1proj[0, 0, 1, :2, :2] == approx(
