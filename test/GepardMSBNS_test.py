...
# Testing gepard MSBAR NLO code - nonsinglet case
...
# For evolution, see last two tests,
# corresponding to Fig. 8, panel 2
# (Numbers for comparison produced by evolutNS)
...
def test_radMSBARLONS():
    """Non-singlet LO CFF H at input scale"""
    pt.Q2 = 2.5
    pt.t = -1.
    pt.xi = 0.01
...
test_radMSBARLONS.extendedtesting = 1
...
test_radMSBARNLONS.extendedtesting = 1
...
def test_radMSBARLONSevol():
    """Non-singlet LO CFF H evolved"""
    pt.Q2 = 10.
    pt.t = -0.25
    pt.xi = 0.01
    pt.xB = 2*pt.xi/(1.+pt.xi)
    ...
    t.m.g.newcall = 1
    t.m.g.init()
    assert_almost_equal(m.ReH(pt),  -9.5542826624654484)
    assert_almost_equal(m.ImH(pt),  -25.836976993895298)


test_radMSBARLONSevol.gepardsuite = 1
